...
class ZemismartManufClusterBattery(TuyaManufCluster):
    """Manufacturer Specific Cluster for Zemismart covers with battery on DP 13."""
    ...
    def handle_cluster_request(
        ...
        *,
        ...
        # DP 13 (0x020D = 525) = Battery percentage
        # Command ID format: 0x02XX where XX is the DP number
        if tuya_payload.command_id == 525:  # 0x020D
            # Battery value is the last byte in the data array
            battery_pct = tuya_payload.data[-1]
            
            # Update the power configuration cluster directly
            power_cluster = self.endpoint.power
            if power_cluster:
                power_cluster._update_attribute(0x0021, battery_pct * 2)  # Zigbee uses 0-200 range
